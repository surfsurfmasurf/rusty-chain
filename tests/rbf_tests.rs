...
fn rbf_replacement_requires_higher_fee_and_sequence() {
    ...
    let err = mp
        ...
        .to_string();
    assert!(
        err.contains("higher sequence number"),
        "Error should mention sequence: {err}"
    );
    ...
    assert_eq!(
        mp.txs.len(),
        1,
        "Should still only have 1 tx in mempool after replacement"
    );
...
#[test]
fn rbf_rejects_lower_fee_even_with_higher_sequence() {
...
fn rbf_replaces_correct_transaction() {
    ...
    mp.add_tx_checked(Transaction::new("alice", "bob", 1, 0), base)
        .unwrap();
    mp.add_tx_checked(Transaction::new("alice", "bob", 1, 1), base)
        .unwrap();
